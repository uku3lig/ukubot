...
public class PrefixOnMention extends Subsystem {
    ...
    public void onGuildMessageReceived(@NotNull GuildMessageReceivedEvent event) {
        ...
        User bot = Main.getJda().getSelfUser();
        String[] mentions = new String[] {bot.getAsMention(),
                Objects.requireNonNull(event.getGuild().getMember(bot)).getAsMention()};
        if (Arrays.stream(mentions).noneMatch(m -> m.equalsIgnoreCase(event.getMessage().getContentRaw()))) return;
